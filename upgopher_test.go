...
func TestIsSafePath(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := isSafePath(tt.baseDir, tt.userPath)
...
Line 5: Case sensitive test`
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			results, err := searchInFile(testFile, tt.searchTerm, tt.caseSensitive, tt.wholeWord)
			if err != nil {
			...
			var actualMatches []SearchResult
...
func TestSearchInFileNonExistent(t *testing.T) {
	_, err := searchInFile("/nonexistent/file.txt", "test", false, false)
	if err == nil {
...
func TestFormatFileSizeLogic(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			value, unit := formatFileSize(tt.bytes)
